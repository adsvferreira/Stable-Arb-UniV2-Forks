...
from modules.token import Erc20Token
...
class LiquidityPool:
    def __init__(
        ...
        if name:
            ...
            self.name = (
                f"{self.token0.symbol}-{self.token1.symbol} (V2, {100*self.fee.numerator/self.fee.denominator:.2f}%)"
            )
    ...
    def calculate_tokens_in_from_ratio_out(self) -> int:
        ...
        if self._ratio_token0_in:
            self.token0_max_swap = max(
                0,
                int(self.reserves_token1 * self._ratio_token0_in) - int(self.reserves_token0 / (1 - self.fee)),
            )
        else:
        ...
        if self._ratio_token1_in:
            self.token1_max_swap = max(
                0,
                int(self.reserves_token0 * self._ratio_token1_in) - int(self.reserves_token1 / (1 - self.fee)),
            )
        else:
    ...
    def calculate_tokens_in_from_tokens_out(
        ...
        denominator = (reserves_out - token_out_quantity) * (self.fee.denominator - self.fee.numerator)
    ...
    def calculate_tokens_out_from_tokens_in(
        ...
        amount_in_with_fee = token_in_quantity * (self.fee.denominator - self.fee.numerator)
    ...
    def set_swap_target(
        ...
        assert (token_in.address == self.token0.address and token_out.address == self.token1.address) or (
            token_in.address == self.token1.address and token_out.address == self.token0.address
        ), "Tokens must match the two tokens held by this pool!"

        if not silent:
            print(f"{token_in} -> {token_out} @ ({token_in_qty} {token_in} = {token_out_qty} {token_out})")

        if token_in.address == self.token0.address:
            # calculate the ratio of token0/token1 for swap of token0 -> token1
            self._ratio_token0_in = Decimal(str(token_in_qty * 10**token_in.decimals)) / Decimal(
                str(token_out_qty * 10**token_out.decimals)
            )

        if token_in.address == self.token1.address:
            # calculate the ratio of token1/token0 for swap of token1 -> token0
            self._ratio_token1_in = Decimal(str(token_in_qty * 10**token_in.decimals)) / Decimal(
                str(token_out_qty * 10**token_out.decimals)
            )

        self.calculate_tokens_in_from_ratio_out()

    def set_lp_swap_target_ratio(
        self,
        token_in: Erc20Token,
        token_out: Erc20Token,
        silent: bool = False,
        ...
        if token_in.address == self.token0.address and self.token0.target_swap_ratio > 0:
            self._ratio_token0_in = float(self.token0.target_swap_ratio) * 10 ** (
                self.token0.decimals - self.token1.decimals
            )
            if not silent:
                print(f"{token_in} -> {token_out} @ TARGET SWAP RATIO: {self.token0.target_swap_ratio}")
                # print(f"{token_in} -> {token_out} @ RAW TARGET SWAP RATIO: {self._ratio_token0_in}")

        if token_in.address == self.token1.address and self.token1.target_swap_ratio > 0:
            self._ratio_token1_in = float(self.token1.target_swap_ratio) * 10 ** (
                self.token1.decimals - self.token0.decimals
            )
            if not silent:
                print(f"{token_in} -> {token_out} @ TARGET SWAP RATIO: {self.token1.target_swap_ratio}")
                # print(f"{token_in} -> {token_out} @ RAW TARGET SWAP RATIO: {self._ratio_token1_in}")
    ...
    def update_reserves(
        ...
        if self._update_method == "polling" or override_update_method == "polling":
            try:
                reserves0, reserves1, _ = self._contract.getReserves(block_identifier=self.update_block)
            ...
            assert (
                external_token0_reserves is not None and external_token1_reserves is not None
            ...
            if external_token0_reserves == self.reserves_token0 and external_token1_reserves == self.reserves_token1:
            ...
            if not silent:
                ...
                if print_reserves:
                    ...
                    print(f"{self.token1.symbol}: {self.reserves_token1}")
                if print_ratios:
                    print(f"{self.token0.symbol}/{self.token1.symbol}: {self.reserves_token0 / self.reserves_token1}")
                    print(f"{self.token1.symbol}/{self.token0.symbol}: {self.reserves_token1 / self.reserves_token0}")
            self.calculate_tokens_in_from_ratio_out()
            return True
