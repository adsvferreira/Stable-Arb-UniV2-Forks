...
from modules.liquidity_pool import LiquidityPool
...
class MultiLiquidityPool:
    def __init__(
        ...
        assert len(pool_addresses) == len(pool_tokens), "Number of pool addresses and token pairs must match!"

        assert len(pool_addresses) > 1, "Only one LP submitted, use LiquidityPool() instead"
    ...
    def _build_multipool_amounts_out(
        ...
        for i in range(number_of_pools):
            ...
            if not silent:
                print(f"Swap {token_in_quantity} {token_in} for {token_out_quantity} {token_out} via {self._pools[i]}")
